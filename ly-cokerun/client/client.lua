...
CreateThread(function()
  RequestModel(`g_m_m_mexboss_01`)
    ...
    cokeboss = CreatePed(2, `g_m_m_mexboss_01`, 706.36, -966.99, 29.41, 341.7, false, false) -- change here the cords for the ped 
...
CreateThread(function()
    exports['qb-target']:AddTargetModel('g_m_m_mexboss_01', {
        options = {
            { 
                type = "client",
                event = "ly-cokerun:client:start",
                icon = "fas fa-circle",
                label = "Get Job ($1500)",
            },
            { 
                type = "client",
                event = "ly-cokerun:client:reward",
                icon = "fas fa-circle",
                label = "Check Product",
                item = 'coke_cured',
            },
...
function Itemtimemsg()
    ...
    TriggerServerEvent('ly-cokerun:server:givecaseitems')
    QBCore.Functions.Notify(Lang:t("success.case_has_been_unlocked"), 'success')
end
---
RegisterNetEvent('ly-cokerun:client:start', function ()
    if CurrentCops >= Config.MinimumMethJobPolice then
        QBCore.Functions.TriggerCallback("ly-cokerun:server:coolc",function(isCooldown)
            if not isCooldown then
                ...
                QBCore.Functions.Progressbar("start_job", Lang:t('info.talking_to_boss'), 10000, false, true, {
                    ...
                    TriggerServerEvent('ly-cokerun:server:startr')
                    TriggerServerEvent('ly-cokerun:server:coolout')
...
RegisterNetEvent('ly-cokerun:server:runactivate', function()
RunStart()
local DrawCoord = 1
    if DrawCoord == 1 then
...
function spawncase()
    local case = CreateObject(`prop_security_case_01`, 1106.04, -2306.84, 30.12, true,  true, true)
    CreateObject(case)
    SetEntityHeading(case, 22.74)
...
RegisterNetEvent('ly-cokerun:client:items', function()
        ...
        if result then
            ...
            function() -- Success
                ...
                QBCore.Functions.Progressbar("grab_case", Lang:t('info.unlocking_case'), 10000, false, true, {
                    ...
                    TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                    RemoveBlip(case)
                    TriggerServerEvent('ly-cokerun:server:unlock')
...
RegisterNetEvent('ly-cokerun:client:reward', function()
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
        if result then
            ...
            QBCore.Functions.Progressbar("product_check", Lang:t('info.checking_quality'), 7000, false, true, {
                ...
                TriggerServerEvent('ly-cokerun:server:rewardpayout')
